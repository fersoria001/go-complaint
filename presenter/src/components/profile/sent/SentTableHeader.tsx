...
function SentTableHeader({ callback, descriptor }: Props) {
    ...
    useEffect(() => {
        async function search() {
            const results = await Query<ComplaintTypeList>(SentSearchQuery,
                SentSearchTypeList, [descriptor.email, query, ...afterBefore, 10, 0])
            callback(results)
        }
        search()
    }, [query, afterBefore, callback, descriptor.email])
...
export default SentTableHeader