...
func TestRemoveFeedbackReplyCommand_Setup(t *testing.T) {
...
func TestRemoveFeedbackReplyCommand_Execute(t *testing.T) {
	TestRemoveFeedbackReplyCommand_Setup(t)
	ctx := context.Background()
	reg := repositories.MapperRegistryInstance()
	userRepository, ok := reg.Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	...
	for _, v := range mock_data.NewFeedbacks {
		...
		c2 := commands.NewRemoveFeedbackReplyCommand(dbf.Id().String(),
			replyReviewMock.Color, repliesIds)
		err = c2.Execute(ctx)
		assert.Nil(t, err)
		dbf, err = feedbackRepository.Find(ctx, find_feedback.ByComplaintId(complaintId))
		assert.Nil(t, err)
