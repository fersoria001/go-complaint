...
func TestChangePasswordCommand_Setup(t *testing.T) {
...
func TestChangePasswordCommand_Execute(t *testing.T) {
	TestChangePasswordCommand_Setup(t)
	ctx := context.Background()
	repository, ok := repositories.MapperRegistryInstance().Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	for _, v := range mock_data.NewUsers {
		...
		p := user.Password()
		newP := "NewPassword1"
		c := commands.NewChangePasswordCommand(v.UserName, v.Password, newP)
		err = c.Execute(ctx)
		assert.Nil(t, err)
		user, err = repository.Find(ctx, find_user.ByUsername(v.UserName))
		assert.Nil(t, err)
		assert.NotEqual(t, p, user.Password())
		svc := infrastructure.AuthenticationServiceInstance()
		err = svc.AuthenticateUser(ctx, v.UserName, newP, false)
		assert.Nil(t, err)
	}
