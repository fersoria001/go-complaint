...
func TestChangeEnterpriseEmailCommand_Setup(t *testing.T) {
...
func TestChangeEnterpriseEmailCommand_Execute(t *testing.T) {
	TestChangeEnterpriseEmailCommand_Setup(t)
	ctx := context.Background()
	userRepository, ok := repositories.MapperRegistryInstance().Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	...
	for _, v := range mock_data.NewRegisterEnterprises {
		...
		c := commands.NewChangeEnterpriseEmailCommand(dbEnterprise.Id().String(), newEmail)
		err = c.Execute(ctx)
		assert.Nil(t, err)
		dbEnterprise, err = enterpriseRepository.Find(ctx, find_enterprise.ByName(v.Name))
		assert.Nil(t, err)
		assert.Equal(t, newEmail, dbEnterprise.Email())
	}
	t.Cleanup(func() {
		for _, v := range mock_data.NewRegisterEnterprises {
			dbEnterprise, err := enterpriseRepository.Find(ctx, find_enterprise.ByName(v.Name))
