...
func TestChangeEnterpriseWebsiteCommand_Setup(t *testing.T) {
...
func TestChangeEnterpriseWebsiteCommand_Execute(t *testing.T) {
	TestChangeEnterpriseWebsiteCommand_Setup(t)
	ctx := context.Background()
	userRepository, ok := repositories.MapperRegistryInstance().Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	...
	for _, v := range mock_data.NewRegisterEnterprises {
		...
		assert.Nil(t, err)
		newWebsite := "http://www.newWebsite.com"
		c := commands.NewChangeEnterpriseWebsiteCommand(dbEnterprise.Id().String(), newWebsite)
		err = c.Execute(ctx)
		assert.Nil(t, err)
		dbEnterprise, err = enterpriseRepository.Find(ctx, find_enterprise.ByName(v.Name))
		assert.Nil(t, err)
		assert.Equal(t, newWebsite, dbEnterprise.Website())
	}
	t.Cleanup(func() {
		for _, v := range mock_data.NewRegisterEnterprises {
			dbEnterprise, err := enterpriseRepository.Find(ctx, find_enterprise.ByName(v.Name))
