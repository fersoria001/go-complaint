...
func TestRemoveFeedbackCommentCommand_Setup(t *testing.T) {
...
func TestRemoveFeedbackCommentCommand_Execute(t *testing.T) {
	TestRemoveFeedbackCommentCommand_Setup(t)
	ctx := context.Background()
	reg := repositories.MapperRegistryInstance()
	userRepository, ok := reg.Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	...
	for _, v := range mock_data.NewFeedbacks {
		...
		c3 := commands.NewRemoveFeedbackCommentCommand(replyReviewMock.Color, dbf.Id().String())
		err = c3.Execute(ctx)
		assert.Nil(t, err)
		dbf, err = feedbackRepository.Find(ctx, find_feedback.ByComplaintId(complaintId))
		assert.Nil(t, err)
		...
		dbReplyReview, err = dbf.ReplyReview(replyReviewMock.Color)
		assert.Nil(t, err)
		assert.Equal(t, "", dbReplyReview.Review().Comment())
	}
	t.Cleanup(func() {
		dbf, err := feedbackRepository.Find(ctx, find_feedback.ByComplaintId(complaintId))
		assert.Nil(t, err)
