...
func TestPromoteEmployeeCommand_Setup(t *testing.T) {
...
func TestPromoteEmployeeCommand_Execute(t *testing.T) {
	TestPromoteEmployeeCommand_Setup(t)
	ctx := context.Background()
	...
	for _, v := range mock_data.NewHiringProccesses {
		...
		for _, dbHp := range dbHps {
			...
			assert.True(t, ok)
			domain.DomainEventPublisherInstance().Reset()
			c2 := commands.NewPromoteEmployeeCommand(
				employee.ID().String(),
				enterprise.MANAGER.String(),
				dbEmployeeUser.Id().String(),
			)
			err = c2.Execute(ctx)
			assert.Nil(t, err)
			updatedEnterprise, err = enterpriseRepository.Get(ctx, updatedHp.Enterprise().Id())
			...
			employee, ok = updatedEnterprise.Employees().Pop()
			assert.True(t, ok)
			assert.Equal(t, enterprise.MANAGER, employee.Position())
			updatedUser, err = userRepository.Get(ctx, updatedHp.User().Id())
			assert.Nil(t, err)
			assert.GreaterOrEqual(t, len(updatedUser.Authorities()), 1)
			counter = 0
			for k, v := range updatedUser.Authorities() {
				if k == updatedEnterprise.Id() {
					for _, authority := range v {
						if authority.Authority() == enterprise.MANAGER.String() {
							counter += 1
						}
					}
					counter += 1
