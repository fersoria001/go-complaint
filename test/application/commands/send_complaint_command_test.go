...
func TestSendComplaintCommand_Execute(t *testing.T) {
	...
	for _, v := range mock_data.NewComplaints {
		...
		complaintData, err := complaintDataRepository.FindAll(ctx, find_all_complaint_data.ByOwnerId(author.Id()))
		assert.Nil(t, err)
		assert.NotNil(t, complaintData)
		assert.Equal(t, len(complaintData), 1)
		complaintData1, err := complaintDataRepository.FindAll(ctx, find_all_complaint_data.ByOwnerId(receiver.Id()))
		assert.Nil(t, err)
