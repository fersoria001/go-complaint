...
func TestRecoverPasswordCommand_Setup(t *testing.T) {
...
func TestRecoverPasswordCommand_Execute(t *testing.T) {
	TestRecoverPasswordCommand_Setup(t)
	ctx := context.Background()
	repository, ok := repositories.MapperRegistryInstance().Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	for _, v := range mock_data.NewUsers {
		...
		assert.Nil(t, err)
		p := user.Password()
		c := commands.NewRecoverPasswordCommand(v.UserName)
		err = c.Execute(ctx)
		assert.Nil(t, err)
		user, err = repository.Find(ctx, find_user.ByUsername(v.UserName))
		assert.Nil(t, err)
