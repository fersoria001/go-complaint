...
func TestChangeBannerImgCommand_Setup(t *testing.T) {
...
func TestChangeBannerImgCommand_Execute(t *testing.T) {
	TestChangeBannerImgCommand_Setup(t)
	ctx := context.Background()
	userRepository, ok := repositories.MapperRegistryInstance().Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	...
	for _, v := range mock_data.NewRegisterEnterprises {
		...
		c := commands.NewChangeBannerImageCommand(
		...
		assert.Equal(t, url, dbEnterprise.BannerIMG())
	}
	t.Cleanup(func() {
		for _, v := range mock_data.NewRegisterEnterprises {
			dbEnterprise, err := enterpriseRepository.Find(ctx, find_enterprise.ByName(v.Name))
