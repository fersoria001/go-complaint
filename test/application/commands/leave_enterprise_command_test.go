...
func TestLeaveEmployeeCommand_Setup(t *testing.T) {
...
func TestLeaveEmployeeCommand_Execute(t *testing.T) {
	TestLeaveEmployeeCommand_Setup(t)
	ctx := context.Background()
	...
	for _, v := range mock_data.NewHiringProccesses {
		...
		for _, dbHp := range dbHps {
			...
			assert.True(t, ok)
			domain.DomainEventPublisherInstance().Reset()
			c2 := commands.NewLeaveEnterpriseCommand(employee.ID().String())
			err = c2.Execute(ctx)
			assert.Nil(t, err)
			updatedHp, err = hiringProccessRepository.Get(ctx, id)
			assert.Nil(t, err)
			assert.Equal(t, enterprise.LEAVED, updatedHp.Status())
			assert.Equal(t, employee.GetUser().Id(), updatedHp.UpdatedBy().Id())
			updatedEnterprise, err = enterpriseRepository.Get(ctx, updatedHp.Enterprise().Id())
			assert.Nil(t, err)
