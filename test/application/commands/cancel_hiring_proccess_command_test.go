...
func TestCancelHiringProccessCommand_Setup(t *testing.T) {
...
func TestCancelHiringProccessCommand_Execute(t *testing.T) {
	TestCancelHiringProccessCommand_Setup(t)
	ctx := context.Background()
	...
	for _, v := range mock_data.NewHiringProccesses {
		...
		for _, dbHp := range dbHps {
			...
			c1 := commands.NewCancelHiringProccessCommand(dbHp.Id, dbEmployeeUser.Id().String(), cancelationReason)
			err = c1.Execute(ctx)
			assert.Nil(t, err)
			updatedHp, err = hiringProccessRepository.Get(ctx, id)
			assert.Nil(t, err)
			assert.Equal(t, enterprise.CANCELED, updatedHp.Status())
			assert.Equal(t, cancelationReason, updatedHp.Reason())
		}
	}
	t.Cleanup(func() {
		enterpriseRepository, ok := repositories.MapperRegistryInstance().Get("Enterprise").(repositories.EnterpriseRepository)
