...
func TestChangeEnterpriseAddressCommand_Setup(t *testing.T) {
...
func TestChangeEnterpriseAddressCommand_Execute(t *testing.T) {
	TestChangeEnterpriseAddressCommand_Setup(t)
	ctx := context.Background()
	userRepository, ok := repositories.MapperRegistryInstance().Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	...
	for _, v := range mock_data.NewRegisterEnterprises {
		...
		assert.Nil(t, err)
		c := commands.NewChangeEnterpriseAddressCommand(dbEnterprise.Id().String(),
			mock_data.Country1.ID(), mock_data.CountryState1.ID(), mock_data.City1.ID())
		err = c.Execute(ctx)
		assert.Nil(t, err)
		dbEnterprise, err = enterpriseRepository.Find(ctx, find_enterprise.ByName(v.Name))
		assert.Nil(t, err)
		assert.Equal(t, mock_data.Country1.ID(), dbEnterprise.Address().Country().ID())
		assert.Equal(t, mock_data.CountryState1.ID(), dbEnterprise.Address().CountryState().ID())
		assert.Equal(t, mock_data.City1.ID(), dbEnterprise.Address().City().ID())
	}
	t.Cleanup(func() {
		for _, v := range mock_data.NewRegisterEnterprises {
			dbEnterprise, err := enterpriseRepository.Find(ctx, find_enterprise.ByName(v.Name))
