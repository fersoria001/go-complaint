...
func TestComplaintSentForReviewByReceiverIdQuery_Setup(t *testing.T) {
	...
	for _, v := range mock_data.NewComplaints {
		...
		assert.GreaterOrEqual(t, len(complaintData), 2)
		...
		assert.GreaterOrEqual(t, len(complaintData1), 1)
...
func TestComplaintSentForReviewByReceiverIdQuery_Execute(t *testing.T) {
	ctx := context.Background()
	reg := repositories.MapperRegistryInstance()
	userRepository, ok := reg.Get("User").(repositories.UserRepository)
	assert.True(t, ok)
	...
	complaintDataRepository, ok := reg.Get("ComplaintData").(repositories.ComplaintDataRepository)
	assert.True(t, ok)
	TestComplaintSentForReviewByReceiverIdQuery_Setup(t)
	for _, v := range mock_data.NewComplaints {
		receiver, err := recipientsRepository.Find(ctx, find_recipient.ByNameAndEmail(v.Receiver.SubjectName, v.Receiver.SubjectEmail))
		assert.Nil(t, err)
		q := queries.NewComplaintsSentForReviewByReceiverIdQuery(receiver.Id().String())
		c, err := q.Execute(ctx)
		assert.Nil(t, err)
