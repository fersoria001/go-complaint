...
type ComplaintsOfResolvedFeedbacksByEmployeeId struct {
	EmployeeId string `json:"employeeId"`
}

func NewComplaintsOfResolvedFeedbacksByEmployeeId(id string) *ComplaintsOfResolvedFeedbacksByEmployeeId {
	return &ComplaintsOfResolvedFeedbacksByEmployeeId{
		EmployeeId: id,
	}
}

func (q ComplaintsOfResolvedFeedbacksByEmployeeId) Execute(ctx context.Context) ([]*dto.Complaint, error) {
	...
	dbC, err := complaintsRepository.FindAll(ctx, find_all_complaints.ByIdsWithStatus(
		...
		[]string{complaint.IN_HISTORY.String()},
	))
	if err != nil {
